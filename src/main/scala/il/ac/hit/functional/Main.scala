...
import il.ac.hit.functional.config.{
  EnvLoader,
  IEnvLoader,
  ISparkSessionProvider,
  SparkSessionProvider
}
import il.ac.hit.functional.extraction.{
  CommitParser,
  GitExtractor,
  ICommitParser,
  IGitExtractor
...
object Main {
  ...
  /** Application entry point. Populates commit data from GitHub, then computes
    * and writes contributor statistics.
    *
    * @param args
  ...
  /** Orchestrates the full pipeline: load config, extract commits, parse JSON,
  ...
  /** Computes the top contributors from commits.csv using Spark, writes the
    * result to a CSV file, and generates per-contributor commit timeline CSVs.
    */
