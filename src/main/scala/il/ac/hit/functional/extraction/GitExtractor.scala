...
class GitExtractor extends IGitExtractor {
  ...
  override def extractCommits(
    ...
    fetchPages(client, repo, perPage, pages, count, token, Nil).map(mergePages)
  }
  ...
  private def fetchPages(
    ...
    // Create the HTTP request builder: URL, accepted format and our identity
    ...
    // Add our token to the header if available
    ...
    Try(client.send(request, HttpResponse.BodyHandlers.ofString())) match {
      // HTTP request failed, for example, invalid github.com is unavailable
      case Failure(exception) =>
        Left(s"Request to GitHub API failed: ${exception.getMessage}")

      // HTTP request succeeded but we got an error, for example, we get rate limited
      case Success(response) if response.statusCode() != 200 =>
        Left(
          s"GitHub API returned status ${response.statusCode()} for page $page"
        )

      // HTTP request succeeded with no errors,
      // continue the recursion or halt prematurely if we read all available commits
