...
class TopContributorsWriter extends ITopContributorsWriter {
   ...
   */
  override def write(dataset: Dataset[ContributorCount], path: String): Either[String, Unit] = {
    ...
    Try {
      ...
      case scala.util.Failure(exception) => Left(s"Failed to write contributors CSV: ${exception.getMessage}")
    }
  }
}
